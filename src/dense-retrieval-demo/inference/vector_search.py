...
def score_cross(model, tokenizer, query, passages, device, batch_size, max_length):
    ...
    for i in range(0, len(pairs), batch_size):
        ...
        for q, p in batch_pairs:
            q_input, q_mask, _ = tokenizer.encode(q, max_length=max_length)
            p_input, p_mask, _ = tokenizer.encode(p, max_length=max_length)

            combined_ids = q_input + p_input[1:]  # [CLS] query_tokens [SEP] passage_tokens [SEP]
            combined_mask = q_mask + p_mask[1:]
        ...
        with torch.no_grad():
            ...
            batch_scores = F.softmax(outputs.logits)[:, 1].cpu().numpy()
...
def encode_texts(model, tokenizer, device, texts, batch_size=args.batch_size, max_length=args.max_length):
    ...
    for i in range(0, len(texts), batch_size):
        ...
        emb = (last_hidden_state * attention_mask.unsqueeze(-1)).sum(1)
